                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in BLOCX_TESTNET
...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    if (!CheckProofOfWork(b_h, block.GetHash(), block.nBits, consensusParams))
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, bool fCheckMasternodesUpgraded)
{
    ...
    if (pindexPrev == nullptr || pindexPrev->nHeight < params.AutolykosForkHeight) {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (pindex->pprev->nHeight >= consensusparams.BRRHeight) {
...
static int64_t nTimeBLOCX_TESTNETSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    /// BLOCX_TESTNET: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(BLOCX_TESTNET): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // BLOCX_TESTNET : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(BLOCX_TESTNET): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT_LOCK, error("ConnectBlock(BLOCX_TESTNET): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
    ...
    // BLOCX_TESTNET : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(*sporkManager, *governance, *::masternodeSync, block, pindex->nHeight, blockReward, strError)) {
        ...
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(BLOCX_TESTNET): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*sporkManager, *governance, *block.vtx[0], pindex->nHeight, blockReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(BLOCX_TESTNET): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
    }
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeBLOCX_TESTNETSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - BLOCX_TESTNET specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeBLOCX_TESTNETSpecific * MICRO, nTimeBLOCX_TESTNETSpecific * MILLI / nBlocksTotal);
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= params.GetConsensus().AutolykosForkHeight) {
        ...
        if (block.nVersion == 0x05) {
            nbits = GetDecodedValue(nbits);
        }
        if (nbits != GetNextWorkRequired(pindexPrev, &block, consensusParams)) {
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(BLOCX_TESTNET): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
