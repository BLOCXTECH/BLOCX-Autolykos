...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 BLOCX_TESTNET irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on blocx_testnetcentral etc. and do not save them...
...
static void gobject_vote_conf_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"gobject vote-conf",
        "Vote on a governance object by masternode configured in blocx_testnet.conf\n",
        {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("blocx_testnet.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by masternode configured in blocx_testnet.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "blocx_testnet",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "blocx_testnet",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "blocx_testnet",               "gobject",                &gobject,                {} },
    { "blocx_testnet",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
