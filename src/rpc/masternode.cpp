...
static void masternode_list_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"masternodelist",
        ...
        "  Lite           - Print info in JSON format for Lites only\n"
        ...
        "  owneraddress   - Print the masternode owner BLOCX_TESTNET address\n"
        "  payee          - Print the masternode payout BLOCX_TESTNET address (can be additionally filtered,\n"
        "                   partial match)\n"
        "  pubKeyOperator - Print the masternode operator public key\n"
        "  status         - Print masternode status: ENABLED / POSE_BANNED\n"
        "                   (can be additionally filtered, partial match)\n"
        "  votingaddress  - Print the masternode voting BLOCX_TESTNET address\n",
        {
...
static UniValue masternode_count(const JSONRPCRequest& request)
{
    ...
    detailedObj.pushKV("Lite", hpmnObj);
...
static UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (
                ...
                strMode != "status" && strMode != "recent" && strMode != "Lite"))
    {
    ...
    bool showHPMNsOnly = strMode == "Lite";
    ...
    mnList.ForEachMN(false, [&](auto& dmn) {
        ...
        if (showHPMNsOnly && dmn.nType != MnType::Lite) {
            return;
        }
        ...
        } else if (strMode == "json" || strMode == "recent" || strMode == "Lite") {
            std::ostringstream streamInfo;
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "blocx_testnet",               "masternode",             &masternode,             {} },
    { "blocx_testnet",               "masternodelist",         &masternode,             {} },
};
// clang-format on
