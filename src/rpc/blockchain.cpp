...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex, llmq::CChainLocksHandler& clhandler, llmq::CInstantSendManager& isman)
{
    ...
    result.pushKV("nNewNonce", (uint64_t)blockindex->nNewNonce);
    result.pushKV("aHeight", (uint64_t)blockindex->aHeight);
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, llmq::CChainLocksHandler& clhandler, llmq::CInstantSendManager& isman, bool txDetails)
{
    ...
    result.pushKV("nNewNonce", (uint64_t)block.nNewNonce);
    result.pushKV("aHeight", (uint64_t)block.aHeight);
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                    {
                        ...
                        {RPCResult::Type::ARR, "addresses", "array of blocx_testnet addresses",
...
static UniValue getblockstats(const JSONRPCRequest& request)
{
    ...
    ret_all.pushKV("subsidy", GetBlockSubsidy(pindex->pprev->nHeight, Params().GetConsensus()));
