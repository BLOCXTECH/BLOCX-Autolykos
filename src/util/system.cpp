...
const char * const BITCOIN_CONF_FILENAME = "blocx_testnet.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //blocx_testnet-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BLOCX_TESTNETCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BLOCX_TESTNETCore
    // Mac: ~/Library/Application Support/BLOCX_TESTNETCore
    // Unix: ~/.blocx_testnetcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BLOCX_TESTNET";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BLOCX_TESTNET";
#else
    // Unix
    return pathRet / ".blocx_testnet";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty blocx_testnet.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure BLOCX_TESTNET Core copyright is not removed by accident
    if (copyright_devs.find("BLOCX_TESTNET Core") == std::string::npos) {
