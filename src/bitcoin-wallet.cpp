...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s blocx_testnet-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "blocx_testnet-wallet is an offline tool for creating and interacting with BLOCX_TESTNET Core wallet files.\n" +
                                      "By default blocx_testnet-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
                                      "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n" +
                                      "Usage:\n" +
                                     "  blocx_testnet-wallet [options] <command>\n\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `blocx_testnet-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
