...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a BLOCX_TESTNET address"));
    ...
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a BLOCX_TESTNET address").arg(strCoinJoinName));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and blocx_testnet: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BLOCX_TESTNET addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BLOCX_TESTNET addresses"));
    ...
    openAction->setStatusTip(tr("Open a blocx_testnet: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BLOCX_TESTNET command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BLOCX_TESTNET network", "", count));
    } else {
...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    int64_t blockTime = Params().GetConsensus().GetCurrentPowTargetSpacing(headersTipHeight);
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the blocx_testnet logo
