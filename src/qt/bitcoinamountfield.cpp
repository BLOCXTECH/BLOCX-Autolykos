...
public:
    explicit AmountValidator(QObject *parent) :
        QValidator(parent),
        currentUnit(BitcoinUnits::BLOCX_TESTNET) {}
...
public:
    ...
    QSize minimumSizeHint() const override
    {
        ...
        int h = 0;
        int w = GUIUtil::TextWidth(fm, BitcoinUnits::format(BitcoinUnits::BLOCX_TESTNET, BitcoinUnits::maxMoney(), false, BitcoinUnits::separatorAlways));
        w += 2; // cursor blinking space
        w += GUIUtil::blocx_testnetThemeActive() ? 24 : 0; // counteract padding from css
        return QSize(w, h);
    }

private:
    int currentUnit{BitcoinUnits::BLOCX_TESTNET};
