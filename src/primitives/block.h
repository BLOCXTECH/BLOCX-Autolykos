...
public:
    ...
    // uint256 AutolykosHash();
    // std::array<uint8_t, 32> hexToArrayDigest32(const std::string& hex) const;
    // std::vector<uint8_t> uint32ToBytes(uint32_t value) const;
    // std::vector<uint8_t> hexToBytesModifierId(const std::string& hex) const;
