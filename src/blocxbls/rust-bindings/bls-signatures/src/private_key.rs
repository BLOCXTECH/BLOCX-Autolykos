...
impl PrivateKey {
    ...
    pub fn generate_blocx_testnet(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "blocx_testnet_helpers")]
    pub fn generate_blocx_testnet_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_blocx_testnet(rng))
            .collect()
    }
