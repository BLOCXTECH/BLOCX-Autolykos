...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " blocx_testnet-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  blocx_testnet-tx [options] <hex-tx> [commands]  Update hex-encoded blocx_testnet transaction\n" +
            "or:     blocx_testnet-tx [options] -create [commands]   Create hex-encoded blocx_testnet transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded blocx_testnet transaction
