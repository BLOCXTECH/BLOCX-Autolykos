...
*
*   Why is this needed?
*   - In BLOCX_TESTNET some blocks are superblocks, which output much higher amounts of coins
...
bool CMasternodePayments::GetBlockTxOuts(int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& voutMasternodePaymentsRet)
{
    ...
    auto dmnPayee = deterministicMNManager->GetListForBlock(pindex).GetMNPayee(false, MnType::Standard_Masternode);
    if (!dmnPayee) {
        return false;
    }
