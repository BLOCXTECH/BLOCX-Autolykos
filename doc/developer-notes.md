...
- [Development guidelines](#development-guidelines)
    - [General BLOCX_TESTNET Core](#general-blocx_testnet-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BLOCX_TESTNET Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
BLOCX_TESTNET Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/BLOCX_TESTNETTECH/BLOCX_TESTNET/blob/main/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_blocx_testnet
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_blocx_testnet --log_level=test_suite
$ valgrind -v --leak-check=full src/blocx_testnetd -printtoconsole
...
# A coverage report will now be accessible at `./test_blocx_testnet.coverage/index.html`.
```
...
To profile a running blocx_testnetd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep blocx_testnetd` -- sleep 60
```
...
BLOCX_TESTNET Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as BLOCX_TESTNET Core, where everyone uses
...
pay attention to for reviewers of BLOCX_TESTNET Core code.

General BLOCX_TESTNET Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BLOCX_TESTNET Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against BLOCX_TESTNET Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blocx_testnet-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blocx_testnet-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BLOCX_TESTNET Core, the wallet was always
