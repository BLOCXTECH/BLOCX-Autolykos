...
The BLOCX_TESTNET Core daemon can be configured to act as a trusted "border
router", implementing the blocx_testnet wire protocol and relay, making
...
The ZeroMQ feature in BLOCX_TESTNET Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building blocx_testnetd:
...
For instance:
    ...
    $ blocx_testnetd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/blocx_testnetd.tx.raw \
...
These options can also be provided in blocx_testnet.conf.
...
From the perspective of blocx_testnetd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into blocx_testnetd directly. Furthermore, no information is
...
using. BLOCX_TESTNETd appends an up-counting sequence number to each
