...
*Setup instructions for a Gitian build of BLOCX_TESTNET Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BLOCX_TESTNET
...
- [Building BLOCX_TESTNET Core](#building-blocx_testnet-core)
...
Clone the git repositories for BLOCX_TESTNET Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/BLOCX_TESTNETTECH/BLOCX_TESTNET/
...
in the BLOCX_TESTNET Core repository under 'Fetch and create inputs' to install sources which require
...
To build BLOCX_TESTNET Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the BLOCX_TESTNET Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/blocx_testnet/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/BLOCX_TESTNETTECH/BLOCX_TESTNET
...
For example:
```bash
URL=https://github.com/crowning-/blocx_testnet.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit blocx_testnet=${COMMIT} --url blocx_testnet=${URL} ../blocx_testnet/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit blocx_testnet=${COMMIT} --url blocx_testnet=${URL} ../blocx_testnet/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit blocx_testnet=${COMMIT} --url blocx_testnet=${URL} ../blocx_testnet/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the blocx_testnet git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../blocx_testnet/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/blocx_testnet
SIGPATH=/some/root/path/blocx_testnet-detached-sigs

./bin/gbuild --url blocx_testnet=${BTCPATH},signature=${SIGPATH} ../blocx_testnet/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/blocx_testnet-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/blocx_testnet-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/blocx_testnet-osx-build.assert
```
...
[blocx_testnet/gitian.sigs](https://github.com/blocx_testnetpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
