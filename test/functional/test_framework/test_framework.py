...
TMPDIR_PREFIX = "blocx_testnet_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave blocx_testnetds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop blocx_testnetds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use blocx_testnet-cli instead of RPC for all commands")
        parser.add_argument("--blocx_testnetd-arg", dest="blocx_testnetd_extra_args", default=[], action="append",
                            help="Pass extra args to all blocx_testnetd instances")
    ...
    def setup(self):
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/blocx_testnetd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/blocx_testnet-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.blocx_testnetd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: blocx_testnetds were not stopped and may still be running")
    ...
    def dynamically_initialize_datadir(self, chain, node_p2p_port, node_rpc_port):
        ...
        if chain == 'testnet3':
            ...
            chain_name_conf_arg_value = '1'
        with open(os.path.join(data_dir, "blocx_testnet.conf"), 'w', encoding='utf8') as f:
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple blocx_testnetds"""
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as blocx_testnetd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in blocx_testnet.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if blocx_testnetd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("blocx_testnetd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if blocx_testnet-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("blocx_testnet-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if blocx_testnet-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("blocx_testnet-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether blocx_testnet-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether blocx_testnet-wallet was compiled."""
...
class BLOCX_TESTNETTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_blocx_testnet_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, hpmn_count=0):
        ...
        self.set_blocx_testnet_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in blocx_testnet-q-recovery thread
    ...
    def set_blocx_testnet_dip8_activation(self, activate_after_block):
    ...
    def set_blocx_testnet_llmq_test_params(self, llmq_size, llmq_threshold):
    ...
    def remove_masternode(self, idx):
        mn = self.mninfo[idx]
        rawtx = self.nodes[0].createrawtransaction([{"txid": mn.collateral_txid, "vout": mn.collateral_vout}], {self.nodes[0].getnewaddress(): 999.13972})
