...
def blocx_testnethash(s):
    return blocx_testnet_hash.getPoWHash(s)
...
# Objects that map to blocx_testnetd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(blocx_testnethash(r))
            self.hash = encode(blocx_testnethash(r)[::-1], 'hex_codec').decode('ascii')
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(blocx_testnethash(r))
            self.hash = int(encode(blocx_testnethash(r)[::-1], 'hex_codec'), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in blocx_testnetd indicates these should be deserialized as blocks
